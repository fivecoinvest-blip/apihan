...
// Deposits today
$stmt = $pdo->prepare("SELECT COUNT(*) as count, SUM(amount) as total FROM payment_transactions WHERE DATE(created_at) = ? AND status = 'completed'");
$stmt->execute([$today]);
$depositsToday = $stmt->fetch(PDO::FETCH_ASSOC);

// Deposits this month
$stmt = $pdo->prepare("SELECT COUNT(*) as count, SUM(amount) as total FROM payment_transactions WHERE created_at >= ? AND status = 'completed'");
$stmt->execute([$thisMonth]);
$depositsMonth = $stmt->fetch(PDO::FETCH_ASSOC);

// Withdrawals today
$stmt = $pdo->prepare("SELECT COUNT(*) as count, SUM(amount) as total FROM withdrawal_transactions WHERE DATE(created_at) = ? AND status = 'completed'");
$stmt->execute([$today]);
$withdrawalsToday = $stmt->fetch(PDO::FETCH_ASSOC);

// Withdrawals this month
$stmt = $pdo->prepare("SELECT COUNT(*) as count, SUM(amount) as total FROM withdrawal_transactions WHERE created_at >= ? AND status = 'completed'");
$stmt->execute([$thisMonth]);
$withdrawalsMonth = $stmt->fetch(PDO::FETCH_ASSOC);

// Total fees collected
$stmt = $pdo->prepare("SELECT SUM(collection_fee) as collection, SUM(processing_fee) as processing FROM payment_transactions WHERE created_at >= ? AND status = 'completed'");
$stmt->execute([$thisMonth]);
$depositFees = $stmt->fetch(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare("SELECT SUM(collection_fee) as collection, SUM(processing_fee) as processing FROM withdrawal_transactions WHERE created_at >= ? AND status = 'completed'");
$stmt->execute([$thisMonth]);
$withdrawalFees = $stmt->fetch(PDO::FETCH_ASSOC);

// Recent transactions (public view - no sensitive data)
$stmt = $pdo->prepare("\n    SELECT 'deposit' as type, pt.amount, pt.collection_fee, pt.processing_fee, pt.status, pt.created_at, pt.pay_type, u.phone AS request_number\n    FROM payment_transactions pt\n    JOIN users u ON u.id = pt.user_id\n    WHERE pt.created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)\n    UNION ALL\n    SELECT 'withdrawal' as type, wt.amount, wt.collection_fee, wt.processing_fee, wt.status, wt.created_at, wt.pay_type, wt.account AS request_number\n    FROM withdrawal_transactions wt\n    WHERE wt.created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)\n    ORDER BY created_at DESC\n    LIMIT 15\n");
$stmt->execute();
...
<body>
    <div class="container">
        <div class="header">
            <div>
                <h1>💳 WPay Gateway Statistics</h1>
                <p style="color: #718096; font-size: 14px; margin-top: 5px;">Real-time Payment Analytics</p>
            </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card fees">
                ...
                <div class="stat-value">₱<?php 
                    $todayDepositFees = $pdo->query("SELECT SUM(collection_fee + processing_fee) as total FROM payment_transactions WHERE DATE(created_at) = '$today' AND status = 'completed'")->fetch()['total'] ?? 0;
                    $todayWithdrawalFees = $pdo->query("SELECT SUM(collection_fee + processing_fee) as total FROM withdrawal_transactions WHERE DATE(created_at) = '$today' AND status = 'completed'")->fetch()['total'] ?? 0;
                    echo number_format($todayDepositFees + $todayWithdrawalFees, 2);
                ?></div>
                <div class="stat-detail">Admin revenue</div>
            </div>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <small><?php echo htmlspecialchars($txn['pay_type']); ?></small>
                        </td>
                        <td>
                            <small><?php echo htmlspecialchars($txn['request_number'] ?? '-'); ?></small>
                        </td>
